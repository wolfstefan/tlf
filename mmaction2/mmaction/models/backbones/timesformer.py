...
class TimeSformer(nn.Module):
    ...
    Args:
        ...
        use_time_embed (bool): Whether or not to use the time embedding. Defaults to True.
    """
    ...
    def forward(self, x):
        ...
        if self.attention_type != 'space_only':
            ...
            if not self.use_time_embed:
                self.time_embed.data = torch.zeros(size=x.shape, device=self.time_embed.data.device)
