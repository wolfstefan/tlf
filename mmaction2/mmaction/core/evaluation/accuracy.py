...
def multi_top_k_accuracy(scores, labels, labelborders, topk=(1, )):
    """Calculate top k accuracy score for multiclass multilabel training.
    This is a really simple evaluation metric that returns the top-k accuracy of the
    first label for each item in the label list.

    Args:
        scores (list[np.ndarray]): Prediction scores for each class.
        labels (list[list[int]]): Ground truth label lists.
        labelborders (list[tuple[int]]): List of label borders, i.e. where each set of labels begins and ends. Example: [(0, 196), (196, 255)]
        topk (tuple[int]): K value for top_k_accuracy. Default: (1, ).

    Returns:
        list[list[float]]: List of top k accuracy scores for each k for each set of labels.
    """

    accs = []
    for idx, (startidx, endidx) in enumerate(labelborders):
        new_scores = [s[startidx:endidx] for s in scores]
        new_labels = [l[idx] - startidx for l in labels]
        accs.append(top_k_accuracy(new_scores, new_labels, topk))
